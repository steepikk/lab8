...
public class AuthManager {
    ...
    private boolean validateRegister() {
        var userName = true;
        if (loginField.getText().isEmpty() || loginField.getText().length() < 1  || loginField.getText().length() > 40) {
            userName = false;
        }
        ...
        if (!passwordErrors.isEmpty()) {
            String message;
            if (!userName) {
                message = localizator.getKeyString("UserNameInvalid") + "\n\n" + localizator.getKeyString("PasswordBad");
            } else {
            ...
            DialogManager.alert("UserNameInvalid", localizator);
            return false;
        }
        return true;
    }
